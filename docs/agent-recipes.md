# Agent Recipes for `jshook_plugin_script_presets`
...
Recommended split during future preset extraction:

- main agent
  - inspect `src/server/domains/hooks`
  - inspect `page_script_register` / `page_script_run`
  - freeze preset data model
- subagent
  - cluster preset families
  - summarize shared arguments and runtime contracts
